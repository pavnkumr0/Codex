    ...
    bindDocumentClickListener() {
        if (!this.documentClickListener) {
            this.documentClickListener = this.renderer.listen(this.el.nativeElement.ownerDocument, 'click', (event) => {
                // Handle edge cases where the target element is the body or the document itself
                if (this.target === document.body || this.target === document) {
                    return;
                }

                // Handle edge cases where the target element is an ancestor of the clicked element
                let clickedElement = <HTMLElement>event.target;
                while (clickedElement) {
                    if (clickedElement === this.target) {
                        return;
                    }
                    clickedElement = clickedElement.parentElement;
                }

                // If none of the edge cases are met, call the leave method
                this.leave();
            });
        }
    }
        ...
        if (!this.documentKeydownListener) {
            this.zone.runOutsideAngular(() => {
                this.documentKeydownListener = this.renderer.listen(this.el.nativeElement.ownerDocument, 'keydown', (event) => {
                    ...
                    // Handle edge cases where the Esc key is pressed but it's not meant for the target element
                    ...
                    // Handle edge cases where the target element is an ancestor of the focused element
                    let focusedElement = <HTMLElement>document.activeElement;
                    while (focusedElement) {
                        if (focusedElement === this.target) {
                            return;
                        }
